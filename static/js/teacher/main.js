function initMain() {
  ...
  window.infomirApi = {
    ...
    async getTheoryTopics(grade) {
      return apiFetch(`/api/theory?grade=${encodeURIComponent(grade)}`);
    },
    ...
    async getTeacherEarningsHistory() {
      return apiFetch('/api/teacher/earnings-history');
    },
    async createTeacherWithdrawal() {
      return apiFetch('/api/teacher/withdrawals', {
        method: 'POST',
      });
    },
    ...
    async getTeacherGroupsWithStudents() {
      return apiFetch('/api/teacher/groups-with-students');
    },
    async getTeacherStudentDetails(studentId) {
      return apiFetch(`/api/teacher/students/${encodeURIComponent(studentId)}/details`);
    },
    async disconnectTeacherStudent(studentId) {
      return apiFetch(`/api/teacher/students/${encodeURIComponent(studentId)}`, {
        method: 'DELETE',
      });
    },
    async moveTeacherStudent(studentId, sourceGroupId, targetGroupId) {
      return apiFetch(`/api/teacher/students/${encodeURIComponent(studentId)}/move`, {
        method: 'POST',
        body: JSON.stringify({
    ...
    async updateTeacherGroup(groupId, title) {
      return apiFetch(`/api/teacher/groups/${encodeURIComponent(groupId)}`, {
        method: 'PATCH',
        body: JSON.stringify({ title }),
      });
    },
    async deleteTeacherGroup(groupId) {
      return apiFetch(`/api/teacher/groups/${encodeURIComponent(groupId)}`, {
        method: 'DELETE',
      });
    },
  ...
  function getRoleHomePath(role) {
    ...
    if (normalized === 'admin') return ROLE_HOME.admin || ROLE_HOME.teacher;
    return ROLE_HOME[normalized] || ROLE_HOME.student;
  }
  ...
  function applyTopMenuByRole(user) {
    const menu = document.querySelector('#mainNav .menu');
    if (!menu) return;
    const role = normalizeRole(user?.role);
    const shouldHideMenu = role === 'teacher';
    menu.hidden = shouldHideMenu;
    menu.style.display = shouldHideMenu ? 'none' : '';
  }
  ...
  function applyAuthHeader(user) {
    ...
    const profileCabinetLink = userWrap.querySelector('[data-profile-cabinet-link]');
    ...
    if (profileCabinetLink) profileCabinetLink.setAttribute('href', getRoleHomePath(user.role));
    applyTopMenuByRole(user);
  ...
  if (registerForm) {
    registerForm.addEventListener('submit', async (e) => {
      ...
      try {
        await window.infomirApi.auth.register({ name, email, password, grade });
        const loginData = await window.infomirApi.auth.login({ email, password });
        window.infomirApi.auth.setToken(loginData?.access_token || '');
        const me = await window.infomirApi.auth.me();
        try {

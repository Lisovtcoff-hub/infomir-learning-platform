function initMain() {
  ...
  window.infomirApi = {
    ...
    async connectTeacher(inviteCode) {
      return apiFetch('/api/users/me/teacher', {
        method: 'POST',
        body: JSON.stringify({ invite_code: inviteCode }),
      });
    },
    async disconnectTeacher() {
      return apiFetch('/api/users/me/teacher', { method: 'DELETE' });
    },
  ...
  document.addEventListener('click', async (e) => {
    ...
    if (!value) return;
    const label = String(target.getAttribute('data-copy-label') || 'Значение').trim() || 'Значение';
    try {
      await copyText(value);
      showToast('success', `${label} скопирован`, `${value} добавлен в буфер обмена.`);
    } catch (_) {
      showToast('error', 'Ошибка копирования', `Не удалось скопировать: ${label.toLowerCase()}.`);
    }
  });
